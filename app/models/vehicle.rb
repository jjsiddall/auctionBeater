class Vehicle < ActiveRecord::Base
	...
	def getAllVehiclesAndBuses
		...
		if get_current_vehicles
			auctionIDs = [4294501315]
			# auctionIDs = [4294503638]
		else
		 	# auctionIDs = [4293991126]

			auctionIDs = [4293967063,
			4293968782,4293968643,4293968691,4293968605,4293968312,4293970442,4293970287,4293972362,4293972179,4293972216,
			4293972732,4293973264,4293974296,4293976475,4293976638,4293976852,4293980035,4293980967,4293982413,4293983088,
			4293983206,4293984121,4293984129,4293984039,4293984236,4293985848,4293985336,4293985846,4293985925,4293988744,
		...
		auctionIDs.each do |auctionID|
			...
			puts "Auction ID: "+auctionID.to_s+" had "+auction_results.count.to_s+" results; currently at "+all_auction_results.count.to_s+" of ~26245"
		end
